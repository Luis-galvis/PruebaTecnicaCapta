...
export class DateUtils {
  ...
  static async adjustToNearestWorkingTime(date: Date): Promise<Date> {
    ...
    const isBusinessDay = await HolidayUtils.isBusinessDay(adjusted);
    const isWorkingHour = (hour >= 8 && hour < 12) || (hour >= 13 && hour < 17);

    if (!isBusinessDay || !isWorkingHour) {
      do {
